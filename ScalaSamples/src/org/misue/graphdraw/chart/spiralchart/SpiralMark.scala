...
//  private var crntFreq: Int = 0
  private var crntRadius: Int = 0
  
  // Constructor
  node.mark = this
  
  private def sumPos3D(firstDate: Calendar, lastDate: Calendar, showTail: Boolean, showHead: Boolean): Tuple6[Double, Double, Double, Double, Int, Int] = {
    val (tailX, tailY, tailMinZ, tailMaxZ, tailCount) = 
      if (showTail) sumPos3D(node.outEdges.map(_.link), firstDate, lastDate) else (0.0, 0.0, Double.MaxValue, Double.MinValue, 0)
    val (headX, headY, headMinZ, headMaxZ, headCount) = 
      if (showHead) sumPos3D(node.inEdges.map(_.link), firstDate, lastDate) else (0.0, 0.0, Double.MaxValue, Double.MinValue, 0)
    (tailX + headX, tailY + headY, Math.min(tailMinZ, headMinZ), Math.max(tailMaxZ, headMaxZ), tailCount, headCount)
  }
  
  private def sumPos3D(links: List[AbstractLink], firstDate: Calendar, lastDate: Calendar): Tuple5[Double, Double, Double, Double, Int] = {
    ...
    def sumPos3D1(links: List[AbstractLink], sumX: Double, sumY: Double, minZ: Double, maxZ: Double, count: Int): Tuple5[Double, Double, Double, Double, Int] = {
      links match {
        ...
        case edge :: rest if edge.date.after(firstDate) => {
          if (lastDate.before(edge.date))
  ...
  def compNormalPos(firstDate: Calendar, crntDate: Calendar, showTail: Boolean, showHead: Boolean,
      compPrimPos: (Double, Double, Double) => Tuple2[Double, Double]): Unit = {
    prevPosX = nextPosX
    prevPosY = nextPosY
    prevPosZ = nextPosZ
    prevFreq = nextFreq
    val (sumX, sumY, minZ, maxZ, tailFreq, headFreq) = sumPos3D(firstDate, crntDate, showTail, showHead)
    ...
    if (nextFreq > 0) {
      ...
      nextPosZ = (minZ, maxZ)
//      crntRadius = 3 * Math.sqrt(crntFreq).toInt
