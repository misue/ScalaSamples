...
class LayoutMaker(chart: Chart) {
    ...
    def initLayout(area: Dimension): Unit = {
        ...
        val radius = 50 // Math.min(area.getWidth(), area.getHeight()) * 0.35
...
//    }
    ...
    def update(area: Dimension): Unit = {
        chart.itemSet.filterNot(_.pointed).foreach((item) => {
            val x = Math.random * (area.getWidth - itemSize) + itemSize / 2.0
            val y = Math.random * (area.getHeight - itemSize) + itemSize / 2.0
            item.nextPos = new Point2D.Double(x, y)        	
        })
    }
