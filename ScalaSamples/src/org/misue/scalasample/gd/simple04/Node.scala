...
class Node(val label: String) {
    ...
    var r: Point2D = new Point2D.Double(0, 0)
    var v: Point2D = new Point2D.Double(0, 0)
...
//    var dy: Double = 0
    ...
    def draw(g: Graphics2D, showLabel: Boolean, fm: FontMetrics): Unit = {
        val ix: Int = pos.getX.toInt
        val iy: Int = pos.getY.toInt
        g.setColor(nodeColor)
